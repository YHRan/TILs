...
class ReadJson_v1():
    ...
    #             if region['shape_attributes']['user'] == user:
    #                 if list(region['region_attributes'].values())[0] == label:
    ...
    def yield_json_data(self, label, user):
        for key in self.d_json:
            data = self.d_json.get(key)
            for region in data['regions']:
                if region['shape_attributes']['user'] == user:
    ...
    def yield_json_data_1(self, label):
        for key in self.d_json:
            data = self.d_json.get(key)
            for region in data['regions']:
                if list(region['region_attributes'].values())[0] == label:
    ...
    def get_conts_list(self, label, user):
        """
        获取指定类别的轮廓
        """
        conts_ds1 = []
        for x_y_list in self.yield_json_data(label, user):
            conts_ds1.append(x_y_list)
        return conts_ds1

    def get_conts_list_1(self, label):
        """
        获取指定类别的轮廓
        """
        conts_ds1 = []
        for x_y_list in self.yield_json_data_1(label):
            conts_ds1.append(x_y_list)
        return conts_ds1
    ...
    def get_json_label(self):
        ...
        for key in self.d_json:
            ...
            for region in data['regions']:
                ...
                json_list.append(list(region['region_attributes'].values())[0])
    ...
    def get_label_num(self):
        ...
        for key in self.d_json:
            data = self.d_json.get(key)
            for region in data['regions']:
                # print(region['region_attributes'].value)
                # label_num_dict[region['region_attributes']['bone_marrow']] += 1

                label_num_dict[list(region['region_attributes'].values())[0]] += 1
...
class ReadJson_v2():
    ...
    def yield_json_data(self, label):
        ...
        for region in self.d_json['features']:
            if 'label_name' not in region['properties']:
                continue
            else:
                # print('label',label)
                if region['properties']['label_name'] == label:
                    ...
                    if type == 'Point':
                        ...
                        if len(ori_data) > 1:  # 是list的话判断为内嵌结构
                            data = []
                            for temp_data in ori_data:
                                temp_data = np.array(temp_data)[:-1] * [1, -1]
                                temp_data = temp_data.reshape(-1, 1, 2)
                                # emp_data += [temp_data[0]]
                                data.append(np.array(temp_data, dtype=np.int32))
                        else:
                            temp_data = ori_data[0]
                            data = np.array(np.array(temp_data)[:-1] * [1, -1], dtype='int32')
                            data = np.array(data.reshape(-1, 1, 2), dtype=np.int32)
                            # ata += [data[0]]
                    yield data
    ...
    def get_json_label(self):
        ...
        # json_label_list = list(set(json_list))
        return json_label_list
...
class ReadJson_v3():
    ...
    def _json_conts(self, region):
        type = region['geometry']['type']
        ori_data = region['geometry']['coordinates']
        # print('ori_data',ori_data)
        # exit()
        if type == 'Point':
            # data = (np.array(ori_data) * [1, 1]).tolist()
            data = (np.array(ori_data)).tolist()
        elif type == 'Line':
            # data = (np.array(ori_data) * [1, 1]).tolist()
            data = (np.array(ori_data)).tolist()
        elif type == 'Polygon':
            # print('ori_data[0]',ori_data[0])
            # print('1111')
            if len(ori_data) > 1:  # 是list的话判断为内嵌结构
                data = []
                for temp_data in ori_data:
                ...
                temp_data = ori_data[0]
                data = np.array(np.array(temp_data)[:-1] * [1, 1], dtype='int32')
                # data = np.array(np.array(temp_data)[:-1], dtype='int32')
                data = np.array(data.reshape(-1, 1, 2), dtype=np.int32)
                # ata += [data[0]]
        else:
            data = []
    ...
    def yield_json_data(self, label, user=None):
        """
        获取标注轮廓
        :param label: 类别名
        :param user: 标注人
        ...
        for region in self.d_json['features']:
            if 'label_name' not in region['properties']:  # 跳过无类别标签
                continue
            else:
                if user:
                    if region['properties']['annotation_owner'] == user and region['properties']['label_name'] == label:
                        data = self._json_conts(region)
                        yield data
                else:
                    if region['properties']['label_name'] == label:
                        data = self._json_conts(region)
    ...
    def get_conts_list(self, label, user=None):
        """
        获取指定类别的轮廓
        """
        conts_ds1 = []
    ...
    def get_json_user(self):
        ...
        for region in self.d_json['features']:
            ...
            if 'label_name' not in region['properties']:
                ...
                json_list.append(region['properties']['annotation_owner'])
        json_user_list = sorted(list(set(json_list)))  # set() 函数创建一个无序不重复元素集
        return json_user_list

    def get_user_label_num(self):
        data_dict = {}
        for user_name in self.get_json_user():
            data_dict[user_name] = {}
            for label_name in self.get_json_label():
                data_dict[user_name][label_name] = len(self.get_conts_list(label_name, user_name))
        return data_dict
...
class WriteJson_user():
    ...
    # def fill_json(self):
    #     id = 0
    #     # print('self.data_dict.items()',self.data_dict.items())
    #     # exit()
    #     for label_1, conts in self.data_dict.items():
    #         label = label_1[0]
    #         user = label_1[1]
    #         print(user)
    #
    ...
    #
    #             one_cnt_json = {'shape_attributes': {'name': 'polygon', 'anno_id': id, 'user': user,
    ...
    def main(self):
        self.skeleton['_via_img_metadata'][os.path.basename(self.slide_path)] = {
            'filename': os.path.basename(self.slide_path), 'size': 0,
            'regions': []}
        self.fill_json()
        self.save_json(savepath=self.save_path, json_content=self.skeleton)
...
def Json_transformation_v2(label, svs_json_path, debug_path):
    for slide_file_path in get_all_file(svs_json_path, '.json'):
        print(slide_file_path)
        json_read = ReadJson_v2(slide_file_path)
        svs_path = slide_file_path.split('.')[0] + '.svs'
        new_svs_path = slide_file_path.split('.')[0].split('\\')[-1] + '.svs'
        svs_read = TiffImg(svs_path)
        h, w = svs_read.get_img_ds_shape(1)
        print(json_read.get_json_label())
        print(json_read.get_label_num())
        mask_Z = []
        for i in range(len(label)):
            mask_Z.append(np.zeros(shape=(h, w), dtype=np.uint8))

        Z_conts = []
        for j in label:
            Z_conts.append(json_read.get_conts_list(j))

        for k in range(len(label)):
            cv2.drawContours(mask_Z[k], Z_conts[k], -1, 255, -1)

        new_mask = sum(mask_Z[1:])
        new_mask_1 = mask_Z[0] - new_mask

        _, cnts, _ = cv2.findContours(new_mask_1, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

        json_dict = {}
        json_dict[label[0]] = cnts
        WriteJson(debug_path, new_svs_path, json_dict).main()


def Json_transformation_v1(label, all_user, svs_json_path, debug_path):
    for slide_file_path in get_all_file(svs_json_path, '.json'):
        print(slide_file_path)
        json_read = ReadJson_v1(slide_file_path)
        svs_path = slide_file_path.split('.')[0] + '.svs'
        new_svs_path = slide_file_path.split('.')[0].split('\\')[-1] + '.svs'
        svs_read = TiffImg(svs_path)
        h, w = svs_read.get_img_ds_shape(1)
        ...
        for i in range(len(label)):
            ...
            conts = []
            mask_Z.append(mask)
            Z_conts.append(conts)

        for i in range(len(label)):
            for j in range(len(all_user)):
                mask_Z[i].append(np.zeros(shape=(w, h), dtype=np.uint8))
                Z_conts[i].append(json_read.get_conts_list(label[i], all_user[j]))

        for i in range(len(label)):
            for j in range(len(all_user)):
                cv2.drawContours(mask_Z[i][j], Z_conts[i][j], -1, 255, -1)

        Z_deduct_mask = []
        for i in range(len(mask_Z[0])):
            sum_of_elements = 0
            for j in range(1, len(mask_Z)):
                sum_of_elements += mask_Z[j][i]
            Z_deduct_mask.append(sum_of_elements)

        Z_deduct_mask_1 = []
        for i in range(len(mask_Z[0])):
            for j in range(1):
                Z_deduct_mask_1.append(mask_Z[j][i])

        new_mask = []
        for i in range(len(Z_deduct_mask_1)):
            new_mask.append(Z_deduct_mask_1[i] - Z_deduct_mask[i])

        new_cnts = []
        for i in range(len(new_mask)):
            _, cnts, _ = cv2.findContours(new_mask[i], cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
            new_cnts.append(cnts)

        json_dict = {}
        for j in range(len(all_user)):
            json_dict[label[0], all_user[j]] = new_cnts[j]
        WriteJson_user(debug_path, new_svs_path, json_dict).main()


def Json_transformation_v3(label, all_user, svs_json_path, debug_path):
    for slide_file_path in get_all_file(svs_json_path, '.json'):
        print(slide_file_path)
        json_read = ReadJson_v1(slide_file_path)
        svs_path = slide_file_path.split('.')[0] + '.svs'
        ...
        Rectangular_frame = []
        for j in range(len(all_user)):
            Rectangular_frame.append(json_read.get_conts_list(label[-1], all_user[j]))

        for j in range(len(all_user)):
            json_dict[label[-1], all_user[j]] = Rectangular_frame[j]
...
def json_2_change_via_settings_json(label, new_label, json_dir, debug_path):
    json_path_list = get_all_file(json_dir, postfix='.json')
    for json_path in tqdm(json_path_list):
        svs_path = json_path.split('.')[0]
        svs_path_1 = svs_path.split("\\")
        new_svs_path = svs_path_1[-1] + '.svs'
        json_dict = {}

        read_json = ReadJson_v3(json_path)
        all_user = read_json.get_json_user()
        ...
        for i in range(len(all_user)):
            for j in range(len(label)):
                # print('read_json.get_conts_list(label[j], all_user[i])',read_json.get_conts_list(label[j], all_user[i]))
                # exit()
                all_conts[i].append(read_json.get_conts_list(label[j], all_user[i]))

        for i in range(len(all_user)):
            for j in range(len(new_label)):
                json_dict[new_label[j], all_user[i]] = all_conts[i][j]

        WriteJson_user(debug_path, new_svs_path, json_dict).main()


def via_settings_json_change_via_settings_json_user(label, new_label, all_user, json_dir, debug_path):
    json_path_list = get_all_file(json_dir, postfix='.json')
    for json_path in tqdm(json_path_list):
        svs_path = json_path.split('.')[0]
        svs_path_1 = svs_path.split("\\")
        ...
        read_json = ReadJson_v1(json_path)
        for i in range(len(all_user)):
            for j in range(len(label)):
                # print('read_json.get_conts_list(label[j], all_user[i])',read_json.get_conts_list(label[j], all_user[i]))
                # exit()
...
def via_settings_json_change_via_settings_json(label, new_label, json_dir, debug_path):
    json_path_list = get_all_file(json_dir, postfix='.json')
    for json_path in tqdm(json_path_list):
        svs_path = json_path.split('.')[0]
        svs_path_1 = svs_path.split("\\")
        ...
        if len(new_label) > 1:
            all_conts = []
            for j in range(len(label)):
                all_conts.append(read_json.get_conts_list_1(label[j]))

            for j in range(len(label)):
                json_dict[new_label[j]] = all_conts[j]
            WriteJson(debug_path, new_svs_path, json_dict).main()
        else:
            all_conts_1 = []
            mix_together_conts = []
            for j in range(len(label)):
                all_conts_1.append(read_json.get_conts_list_1(label[j]))
            for k in range(len(all_conts_1)):
                for kk in range(len(all_conts_1[k])):
                    mix_together_conts.append(all_conts_1[k][kk])
            json_dict[new_label[0]] = mix_together_conts
            WriteJson(debug_path, new_svs_path, json_dict).main()


def Labeling_group_json_change_via_settings_json(label, new_label, json_dir, debug_path):
    json_path_list = get_all_file(json_dir, postfix='.json')
    for json_path in tqdm(json_path_list):
        svs_path = json_path.split('.')[0]
        svs_path_1 = svs_path.split("\\")
        ...
        read_json = ReadJson_v2(json_path)
        if len(new_label) > 1:
            all_conts = []
            for j in range(len(label)):
                all_conts.append(read_json.get_conts_list(label[j]))
            ...
            mix_together_conts = []
            for j in range(len(label)):
                all_conts_1.append(read_json.get_conts_list(label[j]))
            for k in range(len(all_conts_1)):
                for kk in range(len(all_conts_1[k])):
                    mix_together_conts.append(all_conts_1[k][kk])
            json_dict[new_label[0]] = mix_together_conts
...
if __name__ == '__main__':
    # # # # 2.标注组提供的2.0平台json格式重写成 via_settings老版json格式  带user    无需扣减
    # label = ['大鼠颌下腺标注区域', '颌下腺黏液腺细胞核']
    # # new_label代表最后保存的老版json中将label中的标签重新命名后的标签名字，需要与上面一一对应
    # new_label = ['ROI', 'shangpi']
    label = ['舌下腺腺泡']
    # new_label代表最后保存的老版json中将label中的标签重新命名后的标签名字，需要与上面一一对应
    new_label = ['xianpao']
    # all_user医学组提供的2.0平台的json中user的名字
    # annotation_owner = ['184', '27']
    # json_dir医学组提供的2.0平台json存放位置
    json_dir = r'D:\program\src\save_xianpao\json'
    # debug_path代表转换后的老版json存放位置
    debug_path = r'D:\program\src\save_xianpao\old_json'
    # json_2_change_via_settings_json将医学组提供的2.0版json转换成老版json
    json_2_change_via_settings_json(label, new_label, json_dir, debug_path)
